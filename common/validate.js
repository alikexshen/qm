...
const pwdReg = /^[0-9A-Za-z]{8,18}$/
const paypwdReg = /^\d{6}$/
// const inviteCodeReg = /^[a-zA-Z0-9]{6,16}$/
const inviteCodeReg =/^\d{6}$/
export default {
	...
	ispayPwd: function(val) {
		return paypwdReg.test(val)
	},
		...
		for (let rule of rules) {
			...
			if (!data[rule.name]) {
				...
				if (rule.type === 'required' || rule.required) {
					res = { isOk: false, errmsg: rule.errmsg,type:rule.name }
					if (!res.errmsg) {
			...
			switch (rule.type) {
				...
				case 'eq':
					if (data[rule.name] !== data[rule.eqName]) {
						res = { isOk: false, errmsg: rule.errmsg, type:'eq' }
					}
				break
				case 'number':
					if (!numberReg.test(data[rule.name])) {
						res = { isOk: false, errmsg: rule.errmsg,type:'number' }
					}
				break
				case 'int':
					if (!intReg.test(data[rule.name])) {
						res = { isOk: false, errmsg: rule.errmsg,type:'int' }
					}
				break
				case 'phone':
					if (!phoneReg.test(data[rule.name])) {
						res = { isOk: false, errmsg: rule.errmsg ,type:'phone'}
					}
				break
				case 'paypwd':
				console.log(paypwdReg,data[rule.name],3)
					if (!paypwdReg.test(data[rule.name])) {
						res = { isOk: false, errmsg: rule.errmsg ,type:'pwd'}
					}
				break
				case 'email':
				...
				case 'inviteCode':
					if (!inviteCodeReg.test(data[rule.name])) {
						res = { isOk: false, errmsg: rule.errmsg,type:'inviteCode' }
					}
				break
